...
专为**科研论文阅读**设计的 Obsidian 翻译插件。**复制即翻译**——选中术语 → `Ctrl+C` → 自动弹窗显示「**译文 + IPA 音标 + 术语备注 + 读音**」，针对深度学习 / AI / CV / NLP 领域深度优化。

### ✨ 核心特性

| 特性 | 说明 |
|---|---|
| 📋 **复制即翻译** | 选中文字 → `Ctrl+C` → 自动弹窗（**PDF 友好**） |
| 🖱️ **右键翻译** | markdown 编辑器右键 → 「🔬 科研翻译」 |
| 🔤 **IPA 音标** | 自动附带国际音标 |
| 📚 **术语备注** | 模型识别专业术语时附带解释 |
| 🔊 **朗读原文 / 译文** | 浏览器 TTS 朗读 |
| 📄 **PDF 友好** | 复制 PDF 文字自动弹窗 |
| 🖱️ **可拖动弹窗** | 按住标题栏拖动，点击外部关闭 |
| 🎨 **玻璃磨砂 UI** | 圆角 18px + 渐变标题 + 主题自适应 |
